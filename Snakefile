...
rule local_assembly:
    ...
    shell:
        ...
        {VELVETH} {output.assemblies_folder}/$a 63 -shortPaired -fastq -separate {input.small_reads}/$a/$a\_R1.fastq {input.small_reads}/$a/$a\_R2.fastq
        {VELVETG} {output.assemblies_folder}/$a -exp_cov auto -cov_cutoff 8 -max_coverage 100 -scaffolding no

        cp {output.assemblies_folder}/$a/contigs.fa {output.contigs}/$a.fasta
        rm -rf {output.assemblies_folder}/$a/
        }}
